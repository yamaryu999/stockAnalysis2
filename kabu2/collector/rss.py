...
def _make_item_id(source: str, guid: str | None, link: str, title: str, published_at: datetime | None) -> str:
    base_parts = [source or "feed", guid or "", link or "", title or ""]
    if isinstance(published_at, datetime):
        try:
            base_parts.append(published_at.isoformat())
        except Exception:
            pass
    raw = "|".join(base_parts)
...
def parse_feed(url: str, source: str | None = None, *, data: bytes | None = None) -> List[NewsItem]:
    ...
    for e in d.entries:
        ...
        company_name = None
        item_id = _make_item_id(src, guid, link, title, published_at)
        items.append(
            NewsItem(
                id=item_id,
                source=src,
